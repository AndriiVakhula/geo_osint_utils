...
export function parseCoordinates(input: string): Array<number> {
  ...
  if (!hasDirectionLetters) {
    ...
    if (split.length < 2) {
      throw new Error('Неможливо розділити рядок на дві координати');
    }
    const lat = parseFloat(split[0]);
    const lng = parseFloat(split[1]);

    if (Number.isNaN(lat) || Number.isNaN(lng)) {
      throw new Error('Не вдалося перетворити координати у числа');
    }

    if (Math.abs(lat) > 90 || Math.abs(lng) > 180) {
      throw new Error('Некоректний діапазон широти/довготи');
    }
    ...
    if (!matches || matches.length < 2) {
      throw new Error('Не вдалося знайти обидві координати (N/S/E/W)');
    }
    ...
    if (Math.abs(latCandidate) > 90 || Math.abs(lngCandidate) > 180) {
      throw new Error('Недійсне значення широти/довготи.');
    }
